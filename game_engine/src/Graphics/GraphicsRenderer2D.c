...
void drawTimedAnimation(SDL_Renderer **Renderer, const SDL_Rect *targetFrame, SDL_Texture *Texture, int32_t animType,
                    int32_t firstFrame, int32_t MaxFrames, float animSpeed, int Width, int Height, BOOL hFlip, UNUSED int32_t Timer,
                    UNUSED float DeltaTime) {

    for (int32_t AnimFrame = 0; AnimFrame < MaxFrames; AnimFrame++) {
        int32_t currentFrame = ((int32_t) ((SDL_GetTicks() / animSpeed)) % firstFrame);
        SDL_Rect srcFrame = {.x=Width * currentFrame, .y=Height * animType, .w = Width, .h = Height};
        SDL_Rect dstFrame = *targetFrame;
        SDL_RenderCopyEx(*Renderer, Texture, &srcFrame, &dstFrame, 0, NULL, hFlip);
