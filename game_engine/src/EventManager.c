...
void playerEventHandler(BOOL *isRunning, playerActor *Player, float *DeltaTime) {
    ...
    if (!SDL_PollEvent(&gameEvent)) {
        DrawCharacter(IDLE_STATE, DeltaTime, getBaseChar(Player));
    }
    while (SDL_PollEvent(&gameEvent)) {
        switch (gameEvent.type) {
            ...
            case SDL_KEYDOWN:
                keyboardEvent(isRunning, Player, NULL, DeltaTime);
                break;
...
void keyboardEvent(BOOL *isRunning, playerActor *Player, const u_int8_t *gameKeyStates, float *DeltaTime) {
    ...
    while (gameKeyStates[SDL_SCANCODE_UP]) {
        ...
        DrawCharacter(SDL_SCANCODE_UP, DeltaTime, getBaseChar(Player));
        SDL_PumpEvents();
    }
    ...
    while (gameKeyStates[SDL_SCANCODE_RIGHT]) {
        ...
        moveCharacter(getBaseChar(Player), DeltaTime, &Force, &Friction);
        DrawCharacter(SDL_SCANCODE_RIGHT, DeltaTime, getBaseChar(Player));
        SDL_PumpEvents();
    }
    while (gameKeyStates[SDL_SCANCODE_LEFT]) {
        ...
        Force.set(&Force, -2.0, 0.0);
        moveCharacter(getBaseChar(Player), DeltaTime, &Force, &Friction);
        DrawCharacter(SDL_SCANCODE_LEFT, DeltaTime, getBaseChar(Player));
        SDL_PumpEvents();
    }
    while (gameKeyStates[SDL_SCANCODE_DOWN]) {
        ...
        Force.set(&Force, 0.0, 2.0);
        moveCharacter(getBaseChar(Player), DeltaTime, &Force, &Friction);
        DrawCharacter(SDL_SCANCODE_DOWN, DeltaTime, getBaseChar(Player));
        SDL_PumpEvents();
    }
